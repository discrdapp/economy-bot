	...
	async def claim(self, interaction:Interaction):
		...
		with open("claimed.txt", "r") as claimedFile:
			for line in claimedFile:
				if userId in line:
					await interaction.response.send_message("You have already claimed your reward!")
					return
		...
		await interaction.response.send_message(f"Successfully claimed reward! New balance is {bal}")
