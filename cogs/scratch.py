...
class Scratch(commands.Cog):
	...
	@commands.command(description="Play BlackJack!", aliases=['scratchoff'], pass_context=True)
	...
	async def scratch(self, ctx, amntBet, skip:str=""):
		...
		if profit != 0:
			await self.bot.get_cog("Economy").addWinnings(ctx.author.id, amntBet + profit)
